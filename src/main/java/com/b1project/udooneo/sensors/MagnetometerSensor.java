...
public class MagnetometerSensor extends Sensor {
    ...
    public static void enableSensor(boolean enable) throws Exception{
        File file = new File(FileUtils.MAGNETOMETER_ACTIVATION_URI);
    ...
    public static boolean isEnabled(){
        try {
            return Integer.parseInt(read(FileUtils.MAGNETOMETER_ACTIVATION_URI)) == 1;
        } catch (Exception e) {
    ...
    public static String getData(){
        try {
            return read(FileUtils.MAGNETOMETER_DATA_URI);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        return "0,0,0";
